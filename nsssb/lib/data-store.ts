...
let activities: Activity[] = [
  {
    ...
    description: "Organized a blood donation camp at SB College.",
    date: "2025-08-01",
    image: "/images/blood-donation.jpg",
    category: () => "Health",
    location: "SB College Auditorium",
    participants: "Students, Staff, Local Community",
  },
...
class DataStore {
  ...
  getActivities(): Activity[] {
    return this.getData("activities", activities)
  }
...
// Add a new activity (super admin only)
export function addActivity(activity: Activity): void {
  activities.unshift(activity)
}

// Edit an activity (super admin only)
export function editActivity(id: string, updated: Partial<Activity>): boolean {
  const idx = activities.findIndex(a => a.id === id)
  if (idx === -1) return false
  activities[idx] = { ...activities[idx], ...updated }
  return true
}

// Delete an activity (super admin only)
