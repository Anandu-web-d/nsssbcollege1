...
import { useAuth, type UserRole, type Permission, getRoleDisplayName, DEFAULT_PERMISSIONS } from "@/lib/auth"
...
        const superAdminExists = parsedUsers.some((u: User) => 
          u.username === "anandu" && u.role === "super_admin"
        )
        
        if (!superAdminExists) {
          parsedUsers.push({
            id: "super_admin_" + Date.now().toString(), // Generate unique ID
            username: "anandu",
...
          ? { 
              ...u, 
              username: formData.username ?? u.username,
              email: formData.email ?? u.email,
              role: formData.role ?? u.role,
              isActive: formData.isActive ?? u.isActive,
...
      }
      const newUser: User = {
        id: formData.role + "_" + Date.now().toString(), // Generate unique role-based ID
...
      alert("Cannot deactivate super admin user.")
      return
    }
...
// Ensure this is only for development/testing
// localStorage.clear()